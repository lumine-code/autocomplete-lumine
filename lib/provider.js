...
module.exports = {
  ...
  // Suggestions are concatenated in provider order and never re-sorted across
  // providers, so this number alone decides where these land in the list — at
  // the default 1 they sat below every general-purpose provider that also
  // matches a JavaScript file. Ranked above those (the language server and
  // snippets at 2, paths at 3, jupyter-repl at 4) on a narrow claim: this
  // provider answers only a `lumine.` member access inside a Lumine package,
  // and there it is the authoritative source. `spell-check` stays above at
  // 100, which costs nothing — it only ever offers misspellings.
  suggestionPriority: 5,

  // Every other completion provider in the ecosystem declares one. Left at the
  // default 0, this provider is dropped outright by any provider that sets
  // `excludeLowerPriority` with an `inclusionPriority` above zero.
  inclusionPriority: 2,
